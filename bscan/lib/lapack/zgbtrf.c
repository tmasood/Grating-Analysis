...
/*     Determine the block size for this environment */
    ...
    nb = ilaenv_(&c__1, "ZGBTRF", " ", m, n, kl, ku, 6L, 1L);
...
/*        Use unblocked code */
	...
	zgbtf2_(m, n, kl, ku, &AB(1,1), ldab, &IPIV(1), info);
    } else {
...
/*        Use blocked code   
	...
	for (j = 1; j <= nb; ++j) {
	    ...
	    for (i = 1; i <= j-1; ++i) {
		i__3 = i + j * 65 - 66;
		WORK13(i+j*65-66).r = 0., WORK13(i+j*65-66).i = 0.;
/* L10: */
	    }
/* L20: */
	}
...
/*        Zero the subdiagonal elements of the work array WORK31 */
	...
	for (j = 1; j <= nb; ++j) {
	    ...
	    for (i = j + 1; i <= nb; ++i) {
		i__3 = i + j * 65 - 66;
		work31[i+j*65-66].r = 0., work31[i+j*65-66].i = 0.;
/* L30: */
	    }
/* L40: */
	}
...
/*              Find pivot and test for singularity. KM is the
 number of   
   ...
   Computing MIN */
		...
		km = min(i__4,i__5);
		i__4 = km + 1;
		jp = izamax_(&i__4, &AB(kv+1,jj), &c__1);
		IPIV(jj) = jp + jj - j;
		i__4 = kv + jp + jj * ab_dim1;
		if (AB(kv+jp,jj).r != 0. || AB(kv+jp,jj).i != 0.) {
/* Computing MAX   
   Computing MIN */
		    i__6 = jj + *ku + jp - 1;
...
ork array WORK31 */
			    ...
			    i__4 = jj - j;
			    i__5 = *ldab - 1;
			    zswap_(&i__4, &AB(kv+1+jj-j,j), 
				    &i__5, &work31[jp + jj - j - *kl - 1], &
				    c__65);
			    i__4 = j + jb - jj;
			    i__5 = *ldab - 1;
			    i__6 = *ldab - 1;
			    zswap_(&i__4, &AB(kv+1,jj), &i__5, &
				    AB(kv+jp,jj), &i__6);
			}
		    }
...
/*                 Compute multipliers */
		    ...
		    z_div(&z__1, &c_b1, &AB(kv+1,jj));
		    zscal_(&km, &z__1, &AB(kv+2,jj), &c__1);
...
he last column   
   ...
   Computing MIN */
			...
			i__5 = *ldab - 1;
			i__6 = *ldab - 1;
			zgeru_(&km, &i__4, &z__1, &AB(kv+2,jj), 
...
/*              Use ZLASWP to apply the row interchanges to A1
2, A22, and   
                A32. */

		i__3 = *ldab - 1;
		zlaswp_(&j2, &AB(kv+1-jb,j+jb), &i__3, &
			c__1, &jb, &IPIV(j), &c__1);
...
/*                    Update A32 */
			...
			zgemm_("No transpose", "No transpose", &i3, &j2, &jb, 
				&z__1, work31, &c__65, &AB(kv+1-jb,j+jb), &i__3, &c_b1, &AB(kv+*kl+1-jb,j+jb), &i__4)
				;
		    }
		}
...
/*              Copy the current column of A31 back into place
   ...
   Computing MIN */
		...
		nw = min(i__4,i__5);
		if (nw > 0) {
		    zcopy_(&nw, &work31[(jj - j + 1) * 65 - 65], &c__1, &AB(kv+*kl+1-jj+j,jj), &c__1);
		}
/* L170: */
	    }
/* L180: */
