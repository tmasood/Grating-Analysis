...
/* Subroutine */ int zbdsqr_(char *uplo, integer *n, integer *ncvt, integer *
	nru, integer *ncc, doublereal *d, doublereal *e, doublecomplex *vt, 
	integer *ldvt, doublecomplex *u, integer *ldu, doublecomplex *c, 
...
/*     If no singular vectors desired, use qd algorithm */
    ...
    if (! rotate) {
	dlasq1_(n, &D(1), &E(1), &RWORK(1), info);
	return 0;
    }
...
/*        Update singular vectors if desired */
	...
	if (*nru > 0) {
	    zlasr_("R", "V", "F", nru, n, &RWORK(1), &RWORK(*n), &U(1,1),
		     ldu);
	}
	if (*ncc > 0) {
	    zlasr_("L", "V", "F", n, ncc, &RWORK(1), &RWORK(*n), &C(1,1),
		     ldc);
	}
    }
...
/*        Compute singular vectors, if desired */
	...
	if (*ncvt > 0) {
	    zdrot_(ncvt, &VT(m-1,1), ldvt, &VT(m,1), ldvt, &
		    cosr, &sinr);
	}
	if (*nru > 0) {
	    zdrot_(nru, &U(1,m-1), &c__1, &U(1,m), &
		    c__1, &cosl, &sinl);
	}
	if (*ncc > 0) {
	    zdrot_(ncc, &C(m-1,1), ldc, &C(m,1), ldc, &cosl, &
		    sinl);
	}
	m += -2;
	goto L50;
...
/*           Update singular vectors */
	    ...
	    if (*ncvt > 0) {
		i__1 = m - ll + 1;
		zlasr_("L", "V", "F", &i__1, ncvt, &RWORK(1), &RWORK(*n), &VT(ll,1), ldvt);
	    }
	    if (*nru > 0) {
		i__1 = m - ll + 1;
...
/*           Update singular vectors */
	    ...
	    if (*nru > 0) {
		i__1 = m - ll + 1;
		zlasr_("R", "V", "B", nru, &i__1, &RWORK(1), &RWORK(*n), &U(1,ll), ldu);
	    }
	    if (*ncc > 0) {
		i__1 = m - ll + 1;
		zlasr_("L", "V", "B", &i__1, ncc, &RWORK(1), &RWORK(*n), &C(ll,1), ldc);
	    }
...
/*           Change sign of singular vectors, if desired */
	    ...
	    if (*ncvt > 0) {
		zdscal_(ncvt, &c_b71, &VT(i,1), ldvt);
	    }
	}
/* L160: */
    }
...
/*           Swap singular values and vectors */
	    ...
	    D(*n + 1 - i) = smin;
	    if (*ncvt > 0) {
		zswap_(ncvt, &VT(isub,1), ldvt, &VT(*n+1-i,1), ldvt);
	    }
	    if (*nru > 0) {
		zswap_(nru, &U(1,isub), &c__1, &U(1,*n+1-i), &c__1);
	    }
	    if (*ncc > 0) {
		zswap_(ncc, &C(isub,1), ldc, &C(*n+1-i,1), 
			ldc);
	    }
	}
/* L180: */
