...
matnew gnewtcharmatrix(complex<double> *zg, structure *epiptr)
{
  int totallayers;
  int dim, dim2;
  int i, j, ij;
  ...
  calcmoments(order, npanels, nrows, pnls, dmns, &mom);
  trans = 'N';
  fact  = 'E';
  equed = 'B';
  ...
  // compute an LU factorization of a MxN matrix mom[0]. The
  // factorization has the form A = P * L * U
  zgetrf_( &lda, &lda, mom[0], &lda, pvt, &info );
  // estimate the reciprocal of the condition number of a 
  // complex matrix mom[0]
  zgecon_( &nrm, &lda, mom[0], &lda, &anorm, &rcond, wrk, rwrk, &info);

  zgetrs_(&trans, &lda, &lda, mom[0], &lda, pvt, mom[1], &lda, &info);

  // eigval - contains the computed eigenvales
  zgeev_(&jobvl, &jobvr, &ldA, Mom[1], &ldA, eigval, vl, &ldvl,
           vr, &ldvr, wrk, &lwrk, rwrk, &info);
