...
extern int cbesy(complex*, real*, integer*, integer*, complex*, integer*, complex*, integer*);
...
int cfieldez(complex betaroot, complex k0, real nu, real *xF, int *wregion, complex *Qtrans,
	    int nbound, real *typewave, complex coefMat[][MAXSIZE], complex *Ez)
{
  int ztype[DIMENSION];
  real alpha;
  complex z2;
  ...
  while(i < numx)
      ...
      if (typewave[region] > 0)
	{
	  ...
	  if (region == 0)
	    {
	      Ez[i] = c_prod(coefMat[0][region], besjqt[0]);
	    }
	  else
	    {
	      cbesy(&z2, &fnu, &kode, &n, besyqt, &nz, cwrk, &ierr);
	      p1 = c_prod(coefMat[0][region], besjqt[0]);
	      p2 = c_prod(coefMat[1][region], besyqt[0]);
	      p = c_add(p1, p2);
	      Ez[i] = p;
	    }
	  ztype[i] = 19;
	  ...
	  while(1)
	    {
	      ...
	      if (region == 0)
		{
		  Ez[i] = c_prod(coefMat[1][region], besiqt[0]);
		  break;
		}
	      cbesk(&z2, &fnu, &kode, &n, beskqt, &nz, &ierr);
	      if (region == (numr - 1))
		{
		  Ez[i] = c_prod(coefMat[0][region], beskqt[0]);
		  break;
		}
	      
	      p1 = c_prod(coefMat[0][region], beskqt[0]);
	      p2 = c_prod(coefMat[1][region], besiqt[0]);
	      p = c_add(p1, p2);
	      Ez[i] = p;
	      break;
	    }
