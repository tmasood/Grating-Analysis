/* ctrtri.f -- translated by f2c (version 19960717).
			     ...
			     ftnlen diag_len)
{
    ...
    extern /* Subroutine */ int ctrmm_(), ctrsm_();
    static logical upper;
    extern /* Subroutine */ int ctrti2_();
...
/* Writing concatenation */
    ...
    nb = ilaenv_(&c__1, "CTRTRI", ch__1, n, &c_n1, &c_n1, &c_n1, 6L, 2L);
    if (nb <= 1 || nb >= *n) {

/*        Use unblocked code */

	ctrti2_(uplo, diag, n, &a[a_offset], lda, info, 1L, 1L);
    } else {
...
/*              Compute rows 1:j-1 of current block column */
		...
		ctrmm_("Left", "Upper", "No transpose", diag, &i__4, &jb, &
			c_b1, &a[a_offset], lda, &a[j * a_dim1 + 1], lda, 4L, 
			5L, 12L, 1L);
		i__4 = j - 1;
...
/*              Compute inverse of current diagonal block */
		...
		ctrti2_("Upper", diag, &jb, &a[j + j * a_dim1], lda, info, 5L,
			 1L);
/* L20: */
	    }
	} else {
...
lumn */
		    ...
		    ctrmm_("Left", "Lower", "No transpose", diag, &i__1, &jb, 
			    ...
			    + jb + j * a_dim1], lda, 4L, 5L, 12L, 1L);
		    i__1 = *n - j - jb + 1;
		    q__1.r = (float)-1., q__1.i = (float)0.;
		    ctrsm_("Right", "Lower", "No transpose", diag, &i__1, &jb,
			     &q__1, &a[j + j * a_dim1], lda, &a[j + jb + j * 
			    a_dim1], lda, 5L, 5L, 12L, 1L);
		}

/*              Compute inverse of current diagonal block */

		ctrti2_("Lower", diag, &jb, &a[j + j * a_dim1], lda, info, 5L,
			 1L);
/* L30: */
	    }
	}
