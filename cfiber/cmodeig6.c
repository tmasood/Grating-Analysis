...
int main()
{
  ...
  for (i=0; i<100; i++)
    {
      printf("wbeta[%d] = %f bigdetc[%d] = %.25f neff = %.25f\n",
	     i,wbeta[i],i,bigdetc[i],wbeta[i]/k0);
      if (((bigdetc[i] > 0) && (bigdetc[i-1] < 0)) || 
	  ((bigdetc[i] < 0) && (bigdetc[i-1] > 0)) )
	{
  ...
  if (rootfound == 0)
      ...
      for (i=1; i<100; i++)
	{
	  printf("wbeta[%d] = %f bigdetc[%d] = %f neff = %f\n",
		 i,wbeta[i],i,bigdetc[i],wbeta[i]/k0);
	  if (abs(bigdetc[i]) > abs(bigdetc[i-1]))
	    {
	      betaR = (wbeta[i] + wbeta[i-1])/2;
	      wbetah = wbeta[i];
	      wbetal = wbeta[i-1];
  ...
  for (j=0; j<5; j++)
      ...
      for (i=1; i<100; i++)
	{
      ...
      for (i=0; i<100; i++)
	{
	  printf("n-wbeta[%d] = %.25f bigdetc[%d] = %.25f neff = %f\n",
		 i,wbeta[i],i,bigdetc[i],wbeta[i]/k0);
	  if (((bigdetc[i] > 0) && (bigdetc[i-1] < 0)) ||
	      ((bigdetc[i] < 0) && (bigdetc[i-1] > 0)) )
	    {
  ...
  for (i=1; i<200; i++)
    {
      ...
      fprintf(fp,"%f \t %f \t %f \t %f \t %f \t %f \t %f \t %f\n",xF[i],Ez[i],Hz[i],
	      Ephi[i],Hphi[i],Er[i],Hr[i],poyn[i]);
    }
