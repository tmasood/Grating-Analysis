/* cgetri.f -- translated by f2c (version 19960717).
...
int cgetri(integer *n, complex *a, integer *lda, integer *ipiv,
	    complex *work, integer *lwork, integer *info)
{
    ...
    extern /* Subroutine */ int cgemm_(), cgemv_();
    static integer nbmin;
    extern /* Subroutine */ int cswap_(), ctrsm_();
...
/*     Test the input parameters. */
    ...
    *info = 0;
    ...
    lwkopt = *n * nb;
    work[1].r = (real) lwkopt, work[1].i = (float)0.;
    lquery = *lwork == -1;
    if (*n < 0) {
	*info = -1;
    } else if (*lda < max(1,*n)) {
...
/*     and the inverse is not computed. */
    ...
    ctrtri_("Upper", "Non-unit", n, &a[a_offset], lda, info, 5L, 8L);
    if (*info > 0) {
	return 0;
    }
...
os. */
		...
		i__2 = i__ + j * a_dim1;
		a[i__2].r = (float)0., a[i__2].i = (float)0.;
/* L10: */
	    }
...
/*           zeros. */
		...
		for (i__ = jj + 1; i__ <= i__3; ++i__) {
		    ...
		    i__4 = i__ + jj * a_dim1;
		    a[i__4].r = (float)0., a[i__4].i = (float)0.;
/* L30: */
		}
/* L40: */
	    }
...
/*           Compute current block column of inv(A). */
		...
		cgemm_("No transpose", "No transpose", n, &jb, &i__2, &q__1, &
			a[(j + jb) * a_dim1 + 1], lda, &work[j + jb], &ldwork,
			 &c_b2, &a[j * a_dim1 + 1], lda, 12L, 12L);
	    }
	    ctrsm_("Right", "Lower", "No transpose", "Unit", n, &jb, &c_b2, &
		    work[j], &ldwork, &a[j * a_dim1 + 1], lda, 5L, 5L, 12L, 
		    4L);
/* L50: */
	}
    }
...
/* L60: */
    ...
    work[1].r = (real) iws, work[1].i = (float)0.;
    return 0;
